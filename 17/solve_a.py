...
def generate(y, x, d, n):
    ...
    if d is None:
        ...
        candidates = [(y-1, x, "u", 0), (y+1, x, "d", 0)]
        if n < 2:
            candidates.append((y, x + 1, "r", n + 1))
    elif d == "d":
        candidates = [(y, x-1, "l", 0), (y, x+1, "r", 0)]
        if n < 2:
            candidates.append((y + 1, x, "d", n + 1))
    elif d == "l":
        candidates = [(y-1, x, "u", 0), (y+1, x, "d", 0)]
        if n < 2:
            candidates.append((y, x - 1, "l", n + 1))
    elif d == "u":
        candidates = [(y, x-1, "l", 0), (y, x+1, "r", 0)]
        if n < 2:
            candidates.append((y - 1, x, "u", n + 1))
...
print(min([dist for ((y, x, _, __), dist) in distances.items() if y == len(input) - 1 and x == len(input[0]) - 1]))
